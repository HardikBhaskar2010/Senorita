...
-- 4. INDEXES
-- ============================================
CREATE INDEX IF NOT EXISTS idx_constellations_user
ON constellations(user_name, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_constellation_stars_constellation
ON constellation_stars(constellation_id, order_index);

CREATE INDEX IF NOT EXISTS idx_shooting_stars_to_user
ON shooting_star_wishes(to_user, seen, created_at DESC);
...
BEGIN
  BEGIN
    ALTER PUBLICATION supabase_realtime DROP TABLE public.constellations;
  EXCEPTION WHEN undefined_object THEN NULL;
  END;

  BEGIN
    ALTER PUBLICATION supabase_realtime DROP TABLE public.constellation_stars;
  EXCEPTION WHEN undefined_object THEN NULL;
  END;

  BEGIN
    ALTER PUBLICATION supabase_realtime DROP TABLE public.shooting_star_wishes;
  EXCEPTION WHEN undefined_object THEN NULL;
  END;
END $$;
...
'Kiss constellations created by Cookie and Senorita';

COMMENT ON TABLE constellation_stars IS
'Individual stars in constellations with musical notes';

COMMENT ON TABLE shooting_star_wishes IS
'Wishes sent via shooting stars';
