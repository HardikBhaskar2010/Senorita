...
CREATE INDEX IF NOT EXISTS idx_future_memories_order
ON future_memories(order_index);
...
CREATE INDEX IF NOT EXISTS idx_memory_progress_user
ON memory_progress(user_name);

CREATE INDEX IF NOT EXISTS idx_memory_progress_memory
ON memory_progress(memory_id);
...
-- future_memories
DROP POLICY IF EXISTS "Allow all operations on future_memories"
ON future_memories;
...
-- memory_progress
DROP POLICY IF EXISTS "Allow all operations on memory_progress"
ON memory_progress;
...
-- secret_message_unlocks
DROP POLICY IF EXISTS "Allow all operations on secret_message_unlocks"
ON secret_message_unlocks;
...
BEGIN
  BEGIN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.future_memories;
  EXCEPTION WHEN duplicate_object THEN NULL;
  END;

  BEGIN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.memory_progress;
  EXCEPTION WHEN duplicate_object THEN NULL;
  END;

  BEGIN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.secret_message_unlocks;
  EXCEPTION WHEN duplicate_object THEN NULL;
  END;
END $$;
...
BEGIN
  RETURN QUERY
  SELECT
    (SELECT COUNT(*) FROM future_memories),
    (SELECT COUNT(*) FROM memory_progress WHERE user_name = p_user_name),
    CASE 
      WHEN (SELECT COUNT(*) FROM future_memories) > 0 THEN
        ROUND(
          (
            ...
            /
            (SELECT COUNT(*) FROM future_memories)::NUMERIC
          ) * 100,
          2
        )
      ELSE 0
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_visited_memories
...
-- Comments
-- ============================================
COMMENT ON TABLE future_memories IS
'Stores future-themed memory entries with 3D diorama configurations';

COMMENT ON TABLE memory_progress IS
'Tracks which memories each user has visited';

COMMENT ON TABLE secret_message_unlocks IS
'Records when users unlock the final secret message';

COMMENT ON COLUMN future_memories.diorama_config IS
'JSONB configuration for Three.js 3D scene rendering';

COMMENT ON COLUMN future_memories.order_index IS
'Determines the position of memory node along motion path';
